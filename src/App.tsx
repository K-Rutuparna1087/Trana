...
function AuthPage({ onAuth }: any) {
  ...
  const submit = async (e: any) => {
    ...
    const body = isLogin
      ? { email, password }
      : { name, email, password, role: "civilian" };
...
export default function App() {
  ...
  useEffect(() => {
    const u = localStorage.getItem("trana_user");
    const t = localStorage.getItem("trana_token");
  ...
  const fetchIncidents = async () => {
    const res = await fetch(`${API}/api/incidents/active`);
